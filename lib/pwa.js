...
/**
 * Doseg aplikacije je sam dokument, a ne mapa u kojoj leži — vidi poziv
 * u `build-index.mjs`. Dijeljeni doseg znači da prvi instalirani alat
 * proguta sve ostale u istoj mapi.
 *
...
export function manifestFor({ name, short, description, startUrl, scope, theme, background, iconBase }) {
  return `${JSON.stringify({
    // Izričit identitet. Bez njega ga preglednik izvodi iz `start_url`, pa
    // bi promjena polazne adrese ispala nova aplikacija umjesto iste.
    id: startUrl,
    name,
    short_name: short,
    description,
...
/* Klik na obavijest vraća u aplikaciju: ako je već otvorena, samo je
   izvuče u prvi plan; inače je otvori na adresi koju je obavijest ponijela. */
self.addEventListener('notificationclick', (event) => {
  const target = (event.notification.data && event.notification.data.url) || '/';
  const action = event.action;

  // Tipka na obavijesti ne zatvara trajnu obavijest — stranica je sama
  // osvježi u novo stanje. Obični klik na tijelo obavijesti je zatvara.
  if (!action) event.notification.close();

  event.waitUntil((async () => {
    const clients = await self.clients.matchAll({ type: 'window', includeUncontrolled: true });
    const mine = clients.filter((client) => new URL(client.url).pathname === target);

    // Tipka se prosljeđuje stranici; worker ne zna što koji alat radi s njom.
    if (action) {
      for (const client of mine) client.postMessage({ type: 'notification-action', action });
      // Nema žive stranice koja bi je izvela — otvori alat da korisnik preuzme.
      if (!mine.length && self.clients.openWindow) return self.clients.openWindow(target);
      return;
    }

    for (const client of mine) if ('focus' in client) return client.focus();
...
/* ═══════════════════════════════════════════════════════════
   ...
   Ista u katalogu i u svakom alatu, pa je ovdje a ne u njima. Nosi
   vlastite boje umjesto da posuđuje stranične: alati i katalog nemaju
   isti skup varijabli, a tamna pločica s jantarnim gumbom čita se i na
   svijetloj i na tamnoj podlozi.

   Pojavi se jednom. „Ne sad" i uspješna instalacija je gase zauvijek.
   ═══════════════════════════════════════════════════════════ */

const INSTALL_STRINGS = {
  hr: { title: 'Instaliraj kao aplikaciju', yes: 'Instaliraj', no: 'Ne sad',
        ios: 'Otvori Podijeli, pa „Dodaj na početni zaslon".' },
  en: { title: 'Install as an app', yes: 'Install', no: 'Not now',
        ios: 'Open Share, then “Add to Home Screen”.' },
  de: { title: 'Als App installieren', yes: 'Installieren', no: 'Später',
        ios: 'Öffne Teilen, dann „Zum Home-Bildschirm".' }
};

export const INSTALL_BAR = `<script>
(function () {
  'use strict';
  // U okviru ne — katalog alate prikazuje u iframeu, a ondje traka nema smisla.
  if (window.top !== window.self) return;

  var KEY = 'workshop.install';
  ...
  function dismiss() {
    try { localStorage.setItem(KEY, 'off'); } catch (e) { /* privatni način */ }
    var bar = document.getElementById('installBar');
    if (bar) bar.remove();
  }

  function show(withPrompt) {
    if (shown || document.getElementById('installBar')) return;
    shown = true;

    var css = document.createElement('style');
    // --install-offset je jedini dogovor sa stranicom: alat koji drži
    // kontrole pri dnu podigne traku iznad njih. Bez njega je 12 px.
    css.textContent = '#installBar{position:fixed;left:50%;'
      + 'bottom:calc(var(--install-offset, 12px) + env(safe-area-inset-bottom));'
      + 'z-index:9999;display:flex;align-items:center;gap:12px;width:min(520px,calc(100vw - 24px));'
      + 'padding:12px 14px;border-radius:14px;background:#1b1a17;color:#f2efe6;'
      + 'box-shadow:0 14px 40px rgba(0,0,0,.34);transform:translate(-50%,14px);opacity:0;'
      + 'font:400 14px/1.4 system-ui,-apple-system,"Segoe UI",sans-serif;'
      + 'transition:opacity .22s,transform .22s}'
      + '#installBar.on{opacity:1;transform:translate(-50%,0)}'
      + '#installBar .txt{flex:1;min-width:0}'
      + '#installBar b{display:block;font-size:14px;font-weight:600}'
      + '#installBar small{display:block;margin-top:2px;color:#a09a8b;font-size:12px}'
      + '#installBar button{flex:none;border:0;border-radius:9px;cursor:pointer;font:inherit}'
      + '#installBar .yes{padding:9px 14px;background:#d9a441;color:#1b1a17;font-weight:600;font-size:13px}'
      + '#installBar .no{padding:9px 6px;background:none;color:#a09a8b;font-size:13px}'
    ...
    bar.innerHTML = '<span class="txt"><b></b><small></small></span>'
      + '<button type="button" class="no"></button>'
      + '<button type="button" class="yes"></button>';
    bar.querySelector('b').textContent = t.title;
    bar.querySelector('small').textContent = ios ? t.ios : '';
    bar.querySelector('.no').textContent = t.no;

    var yes = bar.querySelector('.yes');
    if (withPrompt) {
      ...
      // Na iPhoneu nema što pozvati — uputa je jedino što možemo dati.
      yes.remove();
    }

    bar.querySelector('.no').addEventListener('click', dismiss);
  ...
  addEventListener('beforeinstallprompt', function (event) {
    ...
    prompt = event;
    setTimeout(function () { show(true); }, 2500);
  });

  addEventListener('appinstalled', dismiss);

  // Safari ne javlja beforeinstallprompt; ondje se ide na uputu.
  if (ios) addEventListener('load', function () { setTimeout(function () { show(false); }, 2500); });
})();
</script>`;
